...
def test_parse_kv_pair(kv_pair_case):
    key, value, encoded, desc = kv_pair_case

    result_key, result_val, offset = payload._parse_kv_pair(encoded, 0)
...
def test_parse_kv_pair_calls_lower_level_functions():
    test_buf = b"\x04test\x04\x2a"  # "test" -> 42

    with (
        patch("spoe_forge.spop.decoders.payloads.decode_string") as mock_decode_string,
        patch("spoe_forge.spop.decoders.payloads.auto_decode_var") as mock_auto_decode,
    ):
        mock_decode_string.return_value = ("test", 5)
        mock_auto_decode.return_value = (42, 7)

        key, val, offset = payload._parse_kv_pair(test_buf, 0)
...
def test_decode_metadata(metadata_case):
    metadata_obj, encoded, desc = metadata_case

    result, offset = payload.decode_metadata(encoded, 0)
...
def test_decode_metadata_calls_decode_int64():
    test_buf = b"\x00\x00\x00\x01\x01\x02"

    with patch("spoe_forge.spop.decoders.payloads.decode_int64") as mock_decode_int64:
        mock_decode_int64.side_effect = [(1, 5), (2, 6)]

        result, offset = payload.decode_metadata(test_buf, 0)
...
def test_decode_metadata_raises_on_short_buffer():
    short_buf = b"\x00\x00"

    with pytest.raises(SpopDecodeError, match="unexpected end of stream"):
        payload.decode_metadata(short_buf, 0)


def test_decode_metadata_raises_on_stream_id_error():
    test_buf = b"\x00\x00\x00\x01\xff\xff"

    with patch("spoe_forge.spop.decoders.payloads.decode_int64") as mock_decode:
        mock_decode.side_effect = SpopDecodeError("test error")

        with pytest.raises(SpopDecodeError, match="error decoding stream_id"):
            payload.decode_metadata(test_buf, 0)


def test_decode_metadata_raises_on_frame_id_error():
    test_buf = b"\x00\x00\x00\x01\x01"

    with (
        patch("spoe_forge.spop.decoders.payloads.decode_int64") as mock_decode,
    ):
...
def test_decode_kv_list(kv_list_case):
    kv_dict, encoded, desc = kv_list_case

    result, offset = payload.decode_kv_list(encoded, 0, len(encoded))

    assert result == kv_dict
    assert offset == len(encoded)


def test_decode_kv_list_empty():
    result, offset = payload.decode_kv_list(b"", 0, 0)

    assert result == {}
    assert offset == 0


def test_decode_kv_list_with_offset():
    buf = b"\xff\xff\x01a\x04\x01"  # Prefix + "a" -> 1
    result, offset = payload.decode_kv_list(buf, 2, 6)

    assert result == {"a": 1}
    assert offset == 6


def test_decode_kv_list_calls_parse_kv_pair():
    test_buf = b"\x01a\x04\x01\x01b\x04\x02"  # "a"->1, "b"->2

    with patch("spoe_forge.spop.decoders.payloads._parse_kv_pair") as mock_parse:
        mock_parse.side_effect = [("a", 1, 4), ("b", 2, 8)]

        result, offset = payload.decode_kv_list(test_buf, 0, 8)

        assert mock_parse.call_count == 2
        assert result == {"a": 1, "b": 2}


def test_decode_list_of_messages(message_list_case):
    messages_dict, encoded, desc = message_list_case

    result, offset = payload.decode_list_of_messages(encoded, 0, len(encoded))
...
def test_decode_list_of_messages_with_offset():
...
def test_decode_list_of_messages_calls_lower_level_functions():
    test_buf = b"\x04test\x01\x01a\x04\x01"  # "test" with 1 arg: "a"->1

    with (
        patch("spoe_forge.spop.decoders.payloads.decode_string") as mock_decode_string,
        patch("spoe_forge.spop.decoders.payloads.decode_tiny_int") as mock_decode_tiny,
        patch("spoe_forge.spop.decoders.payloads._parse_kv_pair") as mock_parse_kv,
    ):
        mock_decode_string.return_value = ("test", 5)
        mock_decode_tiny.return_value = (1, 6)
        mock_parse_kv.return_value = ("a", 1, 10)

        result, offset = payload.decode_list_of_messages(test_buf, 0, 10)
...
def test_decode_list_of_messages_raises_on_duplicate_arg():
    ...
    with pytest.raises(SpopDecodeError, match="unexpected duplicate arg"):
        payload.decode_list_of_messages(test_buf, 0, len(test_buf))


def test_decode_list_of_actions_set_var(action_case):
    action_obj, encoded, desc = action_case

    result, offset = payload.decode_list_of_actions(encoded, 0, len(encoded))
...
def test_decode_list_of_actions_empty():
    result, offset = payload.decode_list_of_actions(b"\x00", 0, 0)
...
def test_decode_list_of_actions_multiple():
    ...
    result, offset = payload.decode_list_of_actions(test_buf, 0, len(test_buf))
...
def test_decode_list_of_actions_calls_parse_kv_pair_for_set_var():
...
def test_decode_list_of_actions_calls_decode_string_for_unset_var():
    test_buf = b"\x02\x02\x02\x04temp"  # UnsetVar(session, "temp")

    with patch("spoe_forge.spop.decoders.payloads.decode_string") as mock_decode:
...
def test_decode_list_of_actions_raises_on_invalid_action_type():
...
def test_decode_list_of_actions_raises_on_invalid_scope():
...
def test_decode_list_of_actions_raises_on_unknown_action():
